...
const DOCKER_PROCESS_TIMEOUT_SOFT: Duration = Duration::from_secs(2);
const DOCKER_PROCESS_TIMEOUT_HARD: Duration = Duration::from_secs(5);
    ...
    let timed_out = match tokio::time::timeout(timeout, command.output()).await {
        Ok(Ok(o)) => {
            ...
            #[cfg(not(target_os = "linux"))]
            {
                ...
                let code = o.lines().next().unwrap_or("").trim().parse().unwrap_or(u32::MAX);
                Ok(ExitStatusExt::from_raw(code))
            }
        }
